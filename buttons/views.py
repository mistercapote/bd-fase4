    ...
    if request.method == "GET":
        if tabela == "usuario":
            with connection.cursor() as cursor:
                ...
                return render(request, 'usuario.html', {'acao': "inserir", 'estados': estados_dict})
        else:
            return render(request, f'{tabela}.html', {'acao': "inserir"})
    elif request.method == "POST":
        query = None
        params = []
        mensagem_sucesso = ""
...
def editar(request, tabela, pk):
    if request.method == "GET":
        with connection.cursor() as cursor:
            cursor.execute(f"SELECT * FROM {tabela} WHERE {tabela}id = {pk[0]};")
        ...
        if tabela == "usuario":
            with connection.cursor() as cursor:
                ...
                estados = cursor.fetchall() 
                estados_dict = [{'estadoid': row[0], 'nomeestado': row[1]} for row in estados]
                return render(request, 'usuario.html', {'acao': 'editar', 'estados': estados_dict})
        else:
            return render(request, f'{tabela}.html', {'acao': 'editar'})
    elif request.method == "POST":
        query = None
        params = []
        mensagem_sucesso = ""
